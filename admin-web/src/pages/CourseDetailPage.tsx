...
export default function CourseDetailPage() {
  ...
  const addQuestionMutation = useMutation({
    mutationFn: ({ areaId, values }: { areaId: string; values: QuestionFormValues }) =>
      apiFetch(`/api/v1/courses/${id}/areas/${areaId}/questions`, { method: 'POST', body: JSON.stringify(values) }),
    onSuccess: () => {
  ...
  const editQuestionMutation = useMutation({
    mutationFn: ({ questionId, values }: { questionId: string; values: QuestionFormValues }) =>
      apiFetch(`/api/v1/courses/${id}/questions/${questionId}`, { method: 'PATCH', body: JSON.stringify(values) }),
    onSuccess: () => {
  ...
  function downloadQuestionsCsvTemplate() {
    const csv = 'question_text,option_a,option_b,option_c,option_d,correct_option,assessment_type\r\n' + 'What is 2+2?,3,4,5,6,b,both\r\n';
